...
<script type="text/javascript">
	function checkMessage(){
		...
		addWorker(name,idcard,age,phone,adress,gender,account);
	}
	
	function addWorker(name,idcard,age,phone,adress,gender,account){
		$.ajax({
			type : "POST",
			url : "<%=request.getContextPath()%>/user/addWorker.do",
			data :{
				name :name,
				idcard : idcard,
				age : age,
...
<body>
	<center>
		...
		<form method="post" id="foreman" action="<%=request.getContextPath()%>/user/enter1.jsp">
					...
					性&nbsp;别&nbsp;：<input type="radio" id="gender" name="gender" value="男"/>男
									<input type="radio" id="gender" name="gender" value="女"/>女<br><br>
