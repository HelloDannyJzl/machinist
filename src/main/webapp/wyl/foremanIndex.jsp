...
<head>
    ...
    <script type="text/javascript">
            ...
            $.jqPaginator('#jqpaginatorid',{
                ...
                onPageChange: function (num, type) {
                	if(type == "change"){
						...
						var flag =  $('#flag').val();
						location.href =
							"<%=request.getContextPath()%>/orders/queryOrdersByForeman.do?projectid="+projectid+"&ordersid="+ordersid+"&flag="+flag+"&totalpage="+totalpage+"&nextpage="+num;
					}
                }
    		});
    	});
    	...
    	*/
    	...
    	function ajaxCloseOrders(ordersid){
    		$.ajax({
    			type:"POST",
    			url:"<%=request.getContextPath()%>/orders/closeOrders.do",
    			data:{
    				ordersid:ordersid
    			},
    			dataType : "json",
    			...
    			success : function(data){
    				if(data.message=="1"){
    					alert("结束订单成功")
    					location.href = "<%=request.getContextPath()%>/orders/queryOrdersByForeman.do";
    				}else{
    					alert("结束订单失败")
    				}
    			},
    	 ...
    	 function queryOrders(){
         	...
         	var ordersid = $('#ordersid').val().trim();
         	var flag = $('#flag').val();
         	location.href = "<%=request.getContextPath()%>/orders/queryOrdersByForeman.do?projectid="+projectid+"&ordersid="+ordersid+"&flag="+flag;
         }																		
...
<body style="padding:10px;">
	...
	<div style="background-color: #AFEEEE;height: 130px;">
			<a style="margin-right: 0px;white-space: normal;display: inline-block;" href="<%=request.getContextPath() %>/user/foremanLogin.jsp"><h2>工头主页</h2></a><h3 style="margin-right: 0px;white-space: normal;display: inline-block;">/工头订单管理</h3>
			<hr>
	</div><br><br>
	<!-- 页面缓存总页数和当前页数 都放在隐藏域里面 -->
		<input type="hidden" id="totalpage" name="totalpage" value="${totalpage}" />
        <input type="hidden" id="currentpage" name="currentpage" value="${currentpage}" />
        <input type="button" value="结束订单" onclick="updateOrders()"/><br><br>
	    <table >
				<td>
					...
					订单状态：<select id = "flag" name = "flag">
								<option value="0" <c:if test="${flag == '0'}"> selected = "selected"</c:if>>请选择状态</option>
