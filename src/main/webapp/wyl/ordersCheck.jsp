...
<head>
    ...
    <script type="text/javascript">
    	...
    	function ajaxCheckOrders(ordersid){
    		$.ajax({
    			type:"post",
    			url :"<%=request.getContextPath()%>/orders/checkOrders.do",
    			data:{
    				ordersid :ordersid
    			},
    			dataType :"json",
...
<body style="padding:10px;">
	...
	<div style="background-color: #AFEEEE;height: 130px;">
			<a style="margin-right: 0px;white-space: normal;display: inline-block;" href="<%=request.getContextPath() %>/user/manage.jsp"><h2>管理员主页面</h2></a><h3 style="margin-right: 0px;white-space: normal;display: inline-block;">/订单管理</h3>
			<hr>
	</div><br><br>
	<!-- 页面缓存总页数和当前页数 都放在隐藏域里面 -->
		<input type="hidden" id="totalpage" name="totalpage" value="${totalpage}" />
        <input type="hidden" id="currentpage" name="currentpage" value="${currentpage}" />
        <input type="hidden" id="likeprojectid" name="likeprojectid" value="${likeprojectid}" />
	    <table >
				<td>
        	...
        	<tr>
        	 ...
        	 <c:forEach items="${queryAllOrders}" var="type">
        	 	<tr>
                    ...
                    <td align="center"height="40px">
                    	<c:if test="${type.flag==0}">
                    	<input type="button" value="审核订单" onclick="ajaxCheckOrders('${type.ordersid}')" style="margin:0px; padding:0px; width:60px; height:30px; line-height:30px;border:0px;"/></c:if>
                    </td>
                   </tr>
        	</c:forEach>
        </table>
